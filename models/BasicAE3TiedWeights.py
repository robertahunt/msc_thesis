...
class BasicAE3TiedWeights(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels=3):
        super(BasicAE3TiedWeights, self).__init__()
        ...
        print('Tying Weights')
        for name in self.encoder._modules:
            if 'c' in name:
                self.encoder._modules[name].weight.data = self.decoder._modules[name].weight.data.transpose(2,3)
            elif ('l' in name):
                self.encoder._modules[name].weight.data = self.decoder._modules[name].weight.data.transpose(0,1)
