...
class Encoder(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels):
        ...
        self.c9 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=4, stride=4)
        self.c10 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=4, stride=4)
        ...
        s9 = calc_size_after_conv(s8, kernel=4, stride=4)
        s10 = calc_size_after_conv(s9, kernel=4, stride=4)
        #s9 = tuple(np.array(s9) -1)
        self.linear_features = s10
        self.l10 = nn.Linear(in_features = np.prod(s10)*128, out_features = hidden_features)
...
class Decoder(BaseModel):
    def __init__(self, pre_flatten_size, hidden_features, latent_features, out_channels):
        ...
        self.c10 = nn.ConvTranspose2d(in_channels=128, out_channels=128, kernel_size=4, stride=4, output_padding=(0,1))
        self.c9 = nn.ConvTranspose2d(in_channels=128, out_channels=128, kernel_size=4, stride=4, output_padding=(0,1))
        ...
        self.p5 = nn.MaxUnpool2d((2,2), stride=2)
        ...
        self.p2 = nn.MaxUnpool2d((2,2), stride=2)
    ...
    def forward(self, x, mp_indices_sizes):
        ...
        x = self.c10(F.relu(x))
        x = self.c9(F.relu(x))
        x = F.interpolate(F.relu(x),size=mp_indices_sizes[2][1][2:])
        x = self.c7(F.relu(x))
        x = self.c6(F.relu(x))
        x = F.interpolate(F.relu(x),size=mp_indices_sizes[1][1][2:])
        x = self.c4(F.relu(x))
        x = self.c3(F.relu(x))
        x = F.interpolate(F.relu(x),size=mp_indices_sizes[0][1][2:])
        x = self.c1(F.relu(x))
        return torch.sigmoid(x)
        
class BasicAutoEncoderMoreLayers1(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels=3):
        super(BasicAutoEncoderMoreLayers1, self).__init__()
