...
class Encoder(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels):
        ...
        self.c0 = nn.Conv2d(in_channels=in_channels, out_channels=64, kernel_size=3)
        self.c1 = nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3)
        ...
        s = imgSize
        s0 = calc_size_after_conv(s, kernel=3)
        s1 = calc_size_after_conv(s0, kernel=3)
    ...
    def forward(self, x):
        ...
        size = x.size()
        self.sizes += [size]
        x = self.m2(F.leaky_relu(x))
        
        x = self.c3(F.leaky_relu(x))
        x = self.c4(F.leaky_relu(x))
        size = x.size()
        self.sizes += [size]
        x = self.m5(F.leaky_relu(x))
        
        x = self.c6(F.leaky_relu(x))
        x = self.c7(F.leaky_relu(x))
        size = x.size()
        self.sizes += [size]
        x = self.m8(F.leaky_relu(x))
        
        x = self.c9(F.leaky_relu(x))
        x = self.c10(F.leaky_relu(x))
        size = x.size()
        self.sizes += [size]
        x = self.m11(F.leaky_relu(x))
        
        self.pre_flatten_size = x.size()
        x = x.view(x.size()[0],-1)
        x = self.l10(F.leaky_relu(x))
        x = self.l11(F.leaky_relu(x))
        return x, self.sizes
...
class Decoder(BaseModel):
    def __init__(self, pre_flatten_size, hidden_features, latent_features, out_channels):
        ...
        self.c1 = nn.ConvTranspose2d(in_channels=64, out_channels=64, kernel_size=3)
        self.c0 = nn.ConvTranspose2d(in_channels=64, out_channels=out_channels, kernel_size=3)
    ...
    def forward(self, x, sizes):
        ...
        x = F.interpolate(F.leaky_relu(x),size=sizes[4][2:])
        x = self.c10(F.leaky_relu(x))
        x = self.c9(F.leaky_relu(x))
        
        x = F.interpolate(F.leaky_relu(x),size=sizes[3][2:])
        x = self.c7(F.leaky_relu(x))
        x = self.c6(F.leaky_relu(x))
        
        x = F.interpolate(F.leaky_relu(x),size=sizes[2][2:])
        x = self.c4(F.leaky_relu(x))
        x = self.c3(F.leaky_relu(x))
        
        x = F.interpolate(F.leaky_relu(x),size=sizes[1][2:])
        x = self.c1(F.leaky_relu(x))
        x = self.c0(F.leaky_relu(x))
        return torch.sigmoid(x)
        
class BasicAutoEncoderMoreLayers3LeakyRelu(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels=3):
        super(BasicAutoEncoderMoreLayers3LeakyRelu, self).__init__()
