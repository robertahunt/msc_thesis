...
class Encoder(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels):
        ...
        self.s = [s0, s1, s2, s3, s4, s5, s6, s7, s8, s9]
...
class Decoder(BaseModel):
    ...
    def forward(self, x, sizes):
        ...
        x = F.interpolate(F.relu(x),output_size=sizes[4][2:])
        x = self.c10(F.relu(x))
        x = self.c9(F.relu(x))
        
        x = F.interpolate(F.relu(x),output_size=sizes[3][2:])
        x = self.c7(F.relu(x))
        x = self.c6(F.relu(x))
...
class BasicAutoEncoderNoInterpolate(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels=3):
        super(BasicAutoEncoderNoInterpolate, self).__init__()
