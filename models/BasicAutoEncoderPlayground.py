...
class Encoder(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels):
        ...
        self.pre_flatten_size = s8
        self.l10 = nn.Linear(in_features = np.prod(s8)*128, out_features = hidden_features)
        self.l11 = nn.Linear(in_features = hidden_features, out_features = latent_features)
        self.s = [s0, s1, s2, s3, s4, s5, s6, s7, s8]
...
class Decoder(BaseModel):
    ...
    def forward(self, x, sizes):
        x = self.l11(x)
        x = self.l10(F.relu(x))
        pre_flatten_shape = torch.Size((x.size()[0],sizes[-1][1],self.pre_flatten_size[0],self.pre_flatten_size[1]))
...
class BasicAutoEncoderPlayground(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, in_channels=3):
        super(BasicAutoEncoderPlayground, self).__init__()
