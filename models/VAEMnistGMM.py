...
class VAEMnistGMM(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features, num_samples, cuda, in_channels, n_gaussians, min_mu, max_mu, sigma, mc_points = 10000):
        super(VAEMnistGMM, self).__init__()
        self._cuda = cuda
        ...
        self.p = make_random_GMM(latent_features, n_gaussians, sigma, min_mu, max_mu, cuda)
    ...
    def plot_preprocess(self, outputs, batch, results, batchSize, cuda):
        num_samples = outputs['mu'].shape[1]
        latent_variables = outputs['mu'].shape[3]
    ...
    def _plot_tsne(self, ax, z_plot, y_plot, classes, colors):
        ...
        g_w, g_mus, g_sigmas = self.g
        z_plot = np.append(z_plot,g_mus.squeeze(3).squeeze(0).cpu().numpy(), axis=0)
        y_plot = np.append(y_plot,['g']*g_mus.shape[1])
        z_tsne = tsne.fit_transform(z_plot)
        for i in range(len(classes)):
            ax.scatter(*z_tsne[y_plot == classes[i]].T, c=colors[i], marker='o', label=classes[i], alpha=0.1)
        ax.scatter(*z_tsne[y_plot == 'g'].T, c='red', marker = 'x', label='g', s=30**2)
        
        ax.legend(np.append(classes,['g']))
    ...
    def forward(self, x): 
        ...
        outputs["x_hat_all"] = x
        outputs["x_hat"] = x_hat
        outputs["z"] = mu.mean(dim=1).squeeze(dim=1)
