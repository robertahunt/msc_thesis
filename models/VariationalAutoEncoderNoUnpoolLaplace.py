...
class VariationalAutoEncoderNoUnpoolLaplace(BaseModel):
    def __init__(self, imgSize, hidden_features, latent_features,num_samples, cuda, in_channels=3):
        super(VariationalAutoEncoderNoUnpoolLaplace, self).__init__()
        self._cuda = cuda
    ...
    def forward(self, x): 
        ...
        with torch.no_grad():
            epsilon = self.laplace.rsample([batch_size, self.num_samples, self.latent_features])            
        ...
        z = mu.unsqueeze(1) + epsilon * b.unsqueeze(1)  
        
        # Run through decoder
        z = z.view(batch_size*self.num_samples, self.latent_features)
